...
def register_employer(request):
    ...
    if request.method == 'POST':
        ...
        if Employer.objects.filter(emp_email=normalized_email):
            ...
            employer = Employer(emp_name=name, emp_phone_number=number, emp_email=normalized_email,
                                emp_password=hashed_password)
            try:
                employer.save()
                return JsonResponse(
                    {"message": "Account Successfully created", "email": normalized_email, "url": "employer_dashboard"},
                    status=200)
